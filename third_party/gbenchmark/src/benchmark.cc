...
void BenchmarkFamilies::FindBenchmarks(
  ...
  for (internal::Benchmark* family : families_) {
    ...
    // TODO(cbraley): There is a clang bug causing a few commented out insert
    // statements to fail here.  I've hardcoded some loops to fix the issue.
    // Remove this once the compiler is fixed or a lib change is made.
    ...
    if (family->rangeX_.empty() && family->rangeY_.empty()) {
      ...
      for (std::size_t i = 0; i < instances.size(); ++i) { benchmarks->push_back(instances[i]); }
